...
typedef struct {
  ...
  const int empty_so_history;

  const int max_children;
  so_func_ptr fwdfn0;
  int fwd_entry_id;
  so_actfunc_ptr *fwd_rules;
  so_dynactfunc_ptr *dyn_fwd_rules;
  int *fwd_prune_table;
  ...
  so_dynactfunc_ptr *dyn_bwd_rules;
  int *bwd_prune_table;
...
#define init_so_history( compiled_game_so_ptr ) (compiled_game_so_ptr)->empty_so_history
...
#define so_fwd_rule_valid_for_history( history, rule_used, compiled_game_so_ptr ) (compiled_game_so_ptr)->fwd_prune_table[(history)+(rule_used)]
...
#define so_bwd_rule_valid_for_history( history, rule_used, compiled_game_so_ptr ) (compiled_game_so_ptr)->bwd_prune_table[(history)+(rule_used)]
