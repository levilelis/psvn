...
static const char *name_of_domain[ NUMDOMAINS ] = { "2" };
...
static const char *fwd_rule_names[ 18 ] = { "rule_1", "rule_2", "rule_3", "rule_4", "rule_5", "rule_6", "rule_7", "rule_8", "rule_9", "rule_10", "rule_11", "rule_12", "rule_13", "rule_14", "rule_15", "rule_16", "rule_17", "rule_18" };
static const char *bwd_rule_names[ 18 ] = { "rule_1", "rule_2", "rule_3", "rule_4", "rule_5", "rule_6", "rule_7", "rule_8", "rule_9", "rule_10", "rule_11", "rule_12", "rule_13", "rule_14", "rule_15", "rule_16", "rule_17", "rule_18" };
...
static int fwd_rule_label_sets[162] = {0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8};

static int bwd_rule_label_sets[162] = {0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8,0,1,2,3,4,5,6,7,8};
...
static void dynfwdrule1( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 1 ] = state->vars[ 1 ];
  child_state->vars[ 2 ] = state->vars[ 2 ];
  child_state->vars[ 3 ] = abst->project_away_var[ 3 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 4 ] = state->vars[ 4 ];
  child_state->vars[ 5 ] = state->vars[ 5 ];
  child_state->vars[ 6 ] = state->vars[ 6 ];
  child_state->vars[ 7 ] = state->vars[ 7 ];
...
static void dynfwdrule2( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 4 ] = state->vars[ 4 ];
  child_state->vars[ 5 ] = state->vars[ 5 ];
  child_state->vars[ 6 ] = abst->project_away_var[ 6 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 7 ] = state->vars[ 7 ];
  child_state->vars[ 8 ] = state->vars[ 8 ];
}
...
static void dynfwdrule7( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 2 ] = state->vars[ 2 ];
  child_state->vars[ 3 ] = abst->project_away_var[ 3 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 4 ] = abst->project_away_var[ 4 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 5 ] = state->vars[ 5 ];
  child_state->vars[ 6 ] = state->vars[ 6 ];
  child_state->vars[ 7 ] = state->vars[ 7 ];
  child_state->vars[ 8 ] = state->vars[ 8 ];
...
static void dynfwdrule8( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 5 ] = state->vars[ 5 ];
  child_state->vars[ 6 ] = abst->project_away_var[ 6 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 7 ] = abst->project_away_var[ 7 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 8 ] = state->vars[ 8 ];
}
...
{
  child_state->vars[ 0 ] = abst->project_away_var[ 0 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 1 ] = abst->project_away_var[ 1 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 2 ] = state->vars[ 2 ];
  child_state->vars[ 3 ] = abst->project_away_var[ 3 ] ? 0 : abst->value_map[0][0];
...
static void dynfwdrule13( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 3 ] = abst->project_away_var[ 3 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 4 ] = abst->project_away_var[ 4 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 5 ] = abst->project_away_var[ 5 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 6 ] = state->vars[ 6 ];
  child_state->vars[ 7 ] = state->vars[ 7 ];
  child_state->vars[ 8 ] = state->vars[ 8 ];
}
...
static void dynfwdrule14( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 6 ] = abst->project_away_var[ 6 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 7 ] = abst->project_away_var[ 7 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 8 ] = abst->project_away_var[ 8 ] ? 0 : abst->value_map[0][1];
}
...
static void dynfwdrule15( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  child_state->vars[ 0 ] = abst->project_away_var[ 0 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 1 ] = abst->project_away_var[ 1 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 2 ] = abst->project_away_var[ 2 ] ? 0 : abst->value_map[0][1];
  child_state->vars[ 3 ] = abst->project_away_var[ 3 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 4 ] = abst->project_away_var[ 4 ] ? 0 : abst->value_map[0][0];
...
static int fwdfn8( const state_t *state, void *next_func )
{
  if( state->vars[ 8 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = fwdfn8_a1_1;
    return 16;
  }
}
...
static int fwdfn5( const state_t *state, void *next_func )
{
  if( state->vars[ 7 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = fwdfn5_a1_1;
    return 10;
  }
}
...
static int fwdfn25_a0_1( const state_t *state, void *next_func )
{
  *((func_ptr *)next_func) = NULL;
  return 9;
}
...
{
  if( state->vars[ 7 ] == 0 ) {
    *((func_ptr *)next_func) = fwdfn25_a0_1;
    return 7;
  } else {
    return -1;
  }
...
static int fwdfn2( const state_t *state, void *next_func )
{
  if( state->vars[ 6 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = fwdfn2_a1_1;
    return 4;
  }
}
...
static int fwdfn38_a0_1( const state_t *state, void *next_func )
{
  *((func_ptr *)next_func) = NULL;
  return 3;
}
...
{
  if( state->vars[ 6 ] == 0 ) {
    *((func_ptr *)next_func) = fwdfn38_a0_1;
    return 1;
  } else {
    return -1;
  }
...
static const int fwd_var_test_table_data[] = {81,2,0,0,0,33,1,2,3,0,0,73,2,2,6,0,0,67,3,2,1,0,0,20,4,2,4,0,0,57,5,2,7,0,0,51,6,2,2,0,0,10,7,2,5,0,0,43,8,2,8,0,0,39,-1,2,8,0,0,42,41,2,5,0,0,12,48,2,8,0,0,45,44,2,8,0,0,-1,46,2,2,0,0,15,14,2,5,0,0,49,-1,2,5,0,0,-1,50,2,7,0,0,56,55,2,2,0,0,19,18,2,8,0,0,53,-1,2,8,0,0,-1,54,2,4,0,0,25,64,2,7,0,0,61,60,2,5,0,0,24,23,2,8,0,0,58,-1,2,8,0,0,-1,59,2,7,0,0,-1,62,2,1,0,0,28,27,2,4,0,0,65,13,2,4,0,0,-1,66,2,6,0,0,72,71,2,1,0,0,32,31,2,7,0,0,69,17,2,7,0,0,-1,70,2,3,0,0,38,80,2,6,0,0,77,76,2,4,0,0,37,36,2,7,0,0,74,22,2,7,0,0,-1,75,2,6,0,0,-1,78,0,16,40,-1,0,17,-1,-1,0,15,-1,-1,0,16,-1,-1,0,14,9,-1,0,13,-1,-1,0,17,-1,-1,0,13,47,-1,0,15,-1,-1,0,12,11,-1,0,14,-1,-1,0,12,-1,-1,0,10,52,-1,0,11,13,-1,0,16,-1,-1,0,13,-1,-1,0,9,17,-1,0,10,-1,-1,0,8,16,-1,0,17,-1,-1,0,15,-1,-1,0,7,22,-1,0,11,-1,-1,0,7,63,-1,0,9,-1,-1,0,6,21,-1,0,8,-1,-1,0,6,-1,-1,0,4,68,-1,0,5,26,-1,0,10,-1,-1,0,7,-1,-1,0,3,30,-1,0,4,-1,-1,0,2,29,-1,0,11,-1,-1,0,9,-1,-1,0,1,35,-1,0,5,-1,-1,0,1,79,-1,0,3,-1,-1,0,0,34,-1};
...
static int bwdfn8_a1_1( const state_t *state, void *next_func )
{
  *((func_ptr *)next_func) = NULL;
  return 15;
}
...
static int bwdfn8( const state_t *state, void *next_func )
{
  if( state->vars[ 8 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = bwdfn8_a1_1;
    return 13;
  }
}
...
static int bwdfn5_a1_1( const state_t *state, void *next_func )
{
...
static int bwdfn5( const state_t *state, void *next_func )
{
  if( state->vars[ 7 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = bwdfn5_a1_1;
    return 7;
  }
}
...
static int bwdfn25_a0_1( const state_t *state, void *next_func )
{
  *((func_ptr *)next_func) = NULL;
  return 11;
}
...
{
  if( state->vars[ 7 ] == 0 ) {
    *((func_ptr *)next_func) = bwdfn25_a0_1;
    return 10;
  } else {
    return -1;
  }
...
static int bwdfn2_a1_1( const state_t *state, void *next_func )
{
...
static int bwdfn2( const state_t *state, void *next_func )
{
  if( state->vars[ 6 ] == 0 ) {
    ...
    *((func_ptr *)next_func) = bwdfn2_a1_1;
    return 1;
  }
}
...
static int bwdfn38_a0_1( const state_t *state, void *next_func )
{
  *((func_ptr *)next_func) = NULL;
  return 5;
}
...
{
  if( state->vars[ 6 ] == 0 ) {
    *((func_ptr *)next_func) = bwdfn38_a0_1;
    return 4;
  } else {
    return -1;
  }
...
static const int bwd_var_test_table_data[] = {81,2,0,0,0,33,1,2,3,0,0,73,2,2,6,0,0,67,3,2,1,0,0,20,4,2,4,0,0,57,5,2,7,0,0,51,6,2,2,0,0,10,7,2,5,0,0,43,8,2,8,0,0,39,-1,2,8,0,0,42,41,2,5,0,0,12,48,2,8,0,0,45,44,2,8,0,0,-1,46,2,2,0,0,15,14,2,5,0,0,49,-1,2,5,0,0,-1,50,2,7,0,0,56,55,2,2,0,0,19,18,2,8,0,0,53,-1,2,8,0,0,-1,54,2,4,0,0,25,64,2,7,0,0,61,60,2,5,0,0,24,23,2,8,0,0,58,-1,2,8,0,0,-1,59,2,7,0,0,-1,62,2,1,0,0,28,27,2,4,0,0,65,13,2,4,0,0,-1,66,2,6,0,0,72,71,2,1,0,0,32,31,2,7,0,0,69,17,2,7,0,0,-1,70,2,3,0,0,38,80,2,6,0,0,77,76,2,4,0,0,37,36,2,7,0,0,74,22,2,7,0,0,-1,75,2,6,0,0,-1,78,0,13,40,-1,0,15,-1,-1,0,17,-1,-1,0,13,-1,-1,0,12,9,-1,0,16,-1,-1,0,15,-1,-1,0,16,47,-1,0,17,-1,-1,0,14,11,-1,0,12,-1,-1,0,14,-1,-1,0,7,52,-1,0,9,13,-1,0,13,-1,-1,0,16,-1,-1,0,11,17,-1,0,7,-1,-1,0,6,16,-1,0,15,-1,-1,0,17,-1,-1,0,10,22,-1,0,9,-1,-1,0,10,63,-1,0,11,-1,-1,0,8,21,-1,0,6,-1,-1,0,8,-1,-1,0,1,68,-1,0,3,26,-1,0,7,-1,-1,0,10,-1,-1,0,5,30,-1,0,1,-1,-1,0,0,29,-1,0,9,-1,-1,0,11,-1,-1,0,4,35,-1,0,3,-1,-1,0,4,79,-1,0,5,-1,-1,0,2,34,-1};
...
{
  if( state->vars[ 0 ] == 0 && state->vars[ 1 ] == 0 && state->vars[ 2 ] == 0 && state->vars[ 3 ] == 0 && state->vars[ 4 ] == 0 && state->vars[ 5 ] == 0 && state->vars[ 6 ] == 1 && state->vars[ 7 ] == 1 && state->vars[ 8 ] == 1 ) {
    return 1;
  }
  return 0;
}
...
const compiled_game_so_t towers3p3d = {
  NUMVARS,
  sizeof( var_t ),
  sizeof( state_t ),
