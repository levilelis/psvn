...
static const char *domain_0[ 3 ] = {"B", "1", "2" };
...
static const char *fwd_rule_names[ 4 ] = { "LEFT", "RIGHT", "LEFT", "RIGHT" };
static const char *bwd_rule_names[ 4 ] = { "LEFT", "RIGHT", "LEFT", "RIGHT" };
static const int fwd_rule_costs[ 4 ] = { 2, 3, 4, 5 };
#define COST_OF_CHEAPEST_FWD_RULE 2
static const int bwd_rule_costs[ 4 ] = { 2, 3, 4, 5 };
#define COST_OF_CHEAPEST_BWD_RULE 2

static int fwd_rule_label_sets[12] = {0,1,2,0,1,2,0,1,2,0,1,2};

static int bwd_rule_label_sets[12] = {0,1,2,0,1,2,0,1,2,0,1,2};

static int fwd_prune_table[ 4 ] = { 0, 0, 0, 0 };
...
static void dynfwdrule2( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  ...
  child_state->vars[ 1 ] = (abst->project_away_var[ 1 ] || abst->project_away_var[ 0 ]) ? state->vars[ 1 ] : state->vars[ abst->fwd_rule_label_sets[ 1 * NUMVARS + 0 ] ];
  child_state->vars[ 2 ] = state->vars[ 2 ];
}
...
static void dynfwdrule4( const state_t *state, state_t *child_state, const abstraction_t* abst )
{
  child_state->vars[ 0 ] = state->vars[ 0 ];
  child_state->vars[ 1 ] = abst->project_away_var[ 1 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 2 ] = (abst->project_away_var[ 2 ] || abst->project_away_var[ 1 ]) ? state->vars[ 2 ] : state->vars[ abst->fwd_rule_label_sets[ 3 * NUMVARS + 1 ] ];
}

static actfunc_ptr fwd_rules[ 4 ] = { fwdrule1, fwdrule2, fwdrule3, fwdrule4 };

static dynactfunc_ptr fwd_dyn_rules[ 4 ] = { dynfwdrule1, dynfwdrule2, dynfwdrule3, dynfwdrule4 };
...
static const int fwd_var_test_table_data[] = {7,2,0,0,0,1,6,2,2,0,2,2,5,2,1,0,0,-1,3,0,1,4,-1,0,2,-1,-1,0,3,2,-1,0,0,1,-1};

static int bwd_prune_table[ 4 ] = { 0, 0, 0, 0 };
...
{
  child_state->vars[ 0 ] = (abst->project_away_var[ 0 ] || abst->project_away_var[ 1 ]) ? state->vars[ 0 ] : state->vars[ abst->bwd_rule_label_sets[ 1 * NUMVARS + 1 ] ];
  child_state->vars[ 1 ] = abst->project_away_var[ 1 ] ? 0 : abst->value_map[0][0];
  child_state->vars[ 2 ] = state->vars[ 2 ];
}
...
{
  child_state->vars[ 0 ] = state->vars[ 0 ];
  child_state->vars[ 1 ] = (abst->project_away_var[ 1 ] || abst->project_away_var[ 2 ]) ? state->vars[ 1 ] : state->vars[ abst->bwd_rule_label_sets[ 3 * NUMVARS + 2 ] ];
  child_state->vars[ 2 ] = abst->project_away_var[ 2 ] ? 0 : abst->value_map[0][0];
}

static actfunc_ptr bwd_rules[ 4 ] = { bwdrule1, bwdrule2, bwdrule3, bwdrule4 };

static dynactfunc_ptr bwd_dyn_rules[ 4 ] = { dynbwdrule1, dynbwdrule2, dynbwdrule3, dynbwdrule4 };
...
static const int bwd_var_test_table_data[] = {7,2,0,0,1,1,6,2,2,0,1,2,5,2,1,0,0,-1,3,0,0,4,-1,0,3,-1,-1,0,2,2,-1,0,1,1,-1};
...
const compiled_game_so_t sliding_tile1x3 = {
  NUMVARS,
  sizeof( var_t ),
  sizeof( state_t ),
