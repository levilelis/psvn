			    ...
			    const PSVN &psvn )
{
  ...
  for( i = 0; i < psvn.numVariables(); ++i ) {
    ...
    string action;
    /* Ignore any '+' that happen to appear in an action. These could
       have been placed there by the user to denote unnessary tests
       when using this rule backwards, but it has no affect on
       actions. */
    if( actions[ i ][ 0 ] == '+' ) {
  ...
  for( i = 0; i < psvn.numRules(); ++i ) {
    ...
    for( j = 0; j < psvn.numVariables(); ++j ) {
      if( psvn.ruleActions( i )[ j ].compare( "-" ) == 0 ) {
        /* action does not change this variable. 
           backwards rule needs to do the same test as the forwards rule,
           so copy the test over.
         */
	if( psvn.ruleTests( i )[ j ][ 0 ] == '+' ) {
          /* test cannot be skipped, so we throw away the '+' symbol. */
	  bwdRuleTests.push_back( psvn.ruleTests( i )[ j ].substr( 1 ) );
	} else {
	  bwdRuleTests.push_back( psvn.ruleTests( i )[ j ] );
	}
        /* note that this is setting the action to '-' */
			...
			vector<PruneTree *> &allNodes )
{
  ...
  m_childPrunedBy.resize( rules.size(), NULL );
  for( i = 0; i < rules.size(); ++i ) {

      if( m_suffix->m_children[ i ] == NULL ) {
          /* we've already pruned rule i */
          
          m_childPrunedBy[i] = m_suffix->m_childPrunedBy[i];
      ...
      if( rules[ i ].applyTo( childValues, childContext ) ) {
          ...
          else 
          {
              ...
              /* note that m_childPrunedBy[i] is set above. */
          }
      } 
      else 
      {
          /* rule is not applicable in this context */
          m_children[ i ] = NULL;
          
          /* mark that this isn't a real prune since the rule is just
             unusable here. */
          m_childPrunedBy[i] = (PruneTree*)RULE_CANNOT_BE_APPLIED;
      }
  }
}
...
void PruneTree::fillTable( vector<int> &table, vector<vector<int> >& seqs,
                           vector<vector<int> >& pruneSeqs,
                           const int depth) {
  size_t i;
  ...
  /* get rule sequence for this node */
  this->ruleSeq(seqs[m_id]);
  seqs[m_id].resize(depth+1, -1);

  for( i = 0; i < m_children.size(); ++i ) {

    if( m_children[ i ] == NULL ) {
        /* child i is pruned */
        assert(m_childPrunedBy[i] != NULL);

        if (m_childPrunedBy[i] == (PruneTree*)RULE_CANNOT_BE_APPLIED) 
        {
            /* child is not actually pruned since this rule could
               never be applied in this context. we insert the value
               -1 here to denote this as pruned (and references the
               empty sequence as justification). However, this table
               entry should never be used at runtime since this rule
               will never be generated in this state. */
            table[ m_id * m_children.size() + i ] = -1;
        }
        else
        {
            vector<int> seq;
            ...
            /* find suffix that causes pruning */
            size_t n = 0;
            vector<int> common;
            PruneTree* node = this;
            const PruneTree* pruner = m_childPrunedBy[i];
            while (n < seq.size() 
                   && node->m_suffix->m_childPrunedBy[i] == pruner) 
            {
                common.push_back(seq[n++]);
                node = node->m_suffix;
            }
            vector<int> other;
            pruner->ruleSeq(other);
           
            /* create equalivent sequence */
            common.insert(common.end(), other.begin(), other.end());
            ASSERT(common.size() <= depth+1);
            common.resize(depth + 1, -1);
            int pruned = pruneSeqs.size() + 1;
            pruneSeqs.push_back(common);

            assert(pruned > 0);
            table[ m_id * m_children.size() + i ]  = -pruned;
        }
    } 
	...
	= m_children[ i ]->m_suffix->m_id * m_children.size();
      m_children[ i ]->fillTable( table, seqs, pruneSeqs, depth );
    } else {
...
void PruneTree::makeTables(vector<int>& table, vector<vector<int> >& seqs,
                           vector<vector<int> >& pruneSeqs,
                           const int depth)
{
    size_t num;
    ...
    seqs.resize( num );
    fillTable( table, seqs, pruneSeqs, depth );
}
...
static void printPruneTable( const vector<int> &table, const string prefix ) {
  ...
  cout << "static const int " << prefix << "_prune_table[ " << table.size() << " ] = {";
...
static void printPruneSeqs(const vector<vector<int> >& table, const string prefix) 
{
    if (table.size() == 0) {
        cout << "static const int " << prefix 
             << "[ " << table.size() << " ][ 0 ] = { };\n\n";
        return;
    }

    cout << "static const int " << prefix << "[ " << table.size() << " ][ "
         << table[0].size() << " ] = {";
...
int main( int argc, char **argv )
{
  ...
  for( int i = 1; i < argc; ++i ) {
    ...
    if(  !strcmp( argv[ i ], "-h" ) || !strcmp( argv[ i ], "--help" ) ) {
      ...
      cout << "or -1 to turn off move pruning" << endl;
      ...
      cout << "values like 1 or 2 are probably best." << endl;
  ...
  cout << "\nstatic const int fwd_history_len = " << fwdHistoryLen << ";\n\n";
  if( fwdHistoryLen >= 0 ) {
      PruneTree *fwdPruneTree = new PruneTree( *game, game->rules(),
                                               fwdHistoryLen );
      vector<int> fwdPruneTable;
      vector<vector<int> > fwdInternalSeqs;
      vector<vector<int> > fwdPruneSeqs;
      fwdPruneTree->makeTables(fwdPruneTable, fwdInternalSeqs, fwdPruneSeqs, fwdHistoryLen);
      fwdPruneTree->destroyTree();
      printPruneTable( fwdPruneTable, string( "fwd" ) );
      printPruneSeqs( fwdInternalSeqs, string( "fwd_internal_sequences" ) );
      printPruneSeqs( fwdPruneSeqs, string( "fwd_prune_sequences" ) );
  } else {
...
//ruleTree->print();
  ...
  if( bwdMoves ) {
    ...
    cout << "\nstatic const int bwd_history_len = " << bwdHistoryLen << ";\n\n";
    if( bwdHistoryLen >= 0) {
        PruneTree *bwdPruneTree = new PruneTree( *game, game->backwardsRules(),
                                                 bwdHistoryLen );
        vector<int> bwdPruneTable;
        vector<vector<int> > bwdInternalSeqs;
        vector<vector<int> > bwdPruneSeqs;
        bwdPruneTree->makeTables(bwdPruneTable, bwdInternalSeqs, bwdPruneSeqs, bwdHistoryLen);
        bwdPruneTree->destroyTree();
        printPruneTable( bwdPruneTable, string( "bwd" ) );
        printPruneSeqs( bwdInternalSeqs, string( "bwd_internal_sequences" ) );
        printPruneSeqs( bwdPruneSeqs, string( "bwd_prune_sequences" ) );
    } else {
  ...
  cout << "#define fwd_rule_valid_for_history( history, rule_used ) (fwd_prune_table[(history)+(rule_used)] >= 0)\n";
  ...
  cout << "/* Returns index of pruning sequence that prunes this rule */\n";
  cout << "#define fwd_rule_pruned_by( history, rule_used ) (-fwd_prune_table[(history)+(rule_used)] - 1)\n\n";
  ...
  if( bwdMoves ) {
    ...
    cout << "/* returns 0 if the rule is pruned, non-zero otherwise */\n";
    cout << "#define bwd_rule_valid_for_history( history, rule_used ) (bwd_prune_table[(history)+(rule_used)] >= 0)\n";
    cout << "/* generate the next history from the current history and a rule */\n";
    cout << "#define next_bwd_history( history, rule_used ) bwd_prune_table[(history)+(rule_used)]\n";
    cout << "/* Returns index of pruning sequence that prunes this rule */\n";
    cout << "#define bwd_rule_pruned_by( history, rule_used ) (-bwd_prune_table[(history)+(rule_used)] - 1)\n\n";
    cout << "\n";
  }
