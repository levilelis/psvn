...
protected:
  ...
  static const int RULE_CANNOT_BE_APPLIED = 0xDEADBEEF;
  ...
  vector<PruneTree *> m_childPrunedBy;
  ...
  void reLabel( size_t *nextLabel ) {
    ...
    void ruleSeq( vector<int>& seq) const {
        seq.clear();
        const PruneTree* current = this;
        while (current->m_parent != NULL) {
            seq.push_back(current->m_parentRuleUsed);
  ...
  /* set the entries in a pruning table */
    void fillTable( vector<int> &table, vector<vector<int> >& seqs, vector<vector<int> >& pruneSeqs, const int depth );
...
public:
  ...
  /* generate tables from the tree for doing move pruning
    will modify the IDs of the nodes */
    void makeTables(vector<int>& t1, vector<vector<int> >& t2, vector<vector<int> >& t3,const int);
};
...
public:
  ...
  void generateDomainCode( ostream &os ) const;
  ...
  string generateGoalCode() const;
  string generateDynGoalCode() const;
  void cmphGenCode(ostream &os);
};
